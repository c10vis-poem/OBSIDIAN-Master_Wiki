Source: Drive file ID `1CyjGCEhv7KwUMKp_gi0hsW83Cem7Q-U5` ("Copy of Copy of multi-agent.html") — category: Claude API / Managed Agents product documentation, saved as a raw rendered HTML page. This is the "Multiagent sessions" reference page from the Managed Agents section of the Claude API docs (canonical URL: `https://platform.claude.com/docs/en/managed-agents/multi-agent`).
...
Related endpoints referenced on this page (link text only, no path given in source): List threads, Interrupt a session thread, Archive a session thread.
...
Critical events are proxied to the primary thread. However, you might still want to investigate a specific agent's reasoning and tool calls. To do so, stream or list the events from the associated session thread (referenced endpoints: Stream session thread events, List session thread events).

### Tool permissions and custom tools

If a subagent needs something from your client, such as [permission to run an `always_ask` tool](https://platform.claude.com/docs/en/managed-agents/events-and-streaming#tool-confirmation), or the [result of a custom tool](https://platform.claude.com/docs/en/managed-agents/events-and-streaming#handling-custom-tool-calls), the event is cross-posted to the primary thread with `session_thread_id` identifying the originating session thread.
...
## Appendix: Managed Agents docs navigation (left sidebar, captured from this page)
...
- Self-hosted sandboxes (nav section header, no direct link captured)

**Delegate work to your agent**
- [Start a session](https://platform.claude.com/docs/en/managed-agents/sessions)
- Session operations (nav section header, no direct link captured)
...
- [Access GitHub](https://platform.claude.com/docs/en/managed-agents/github)
- [Attach and download files](https://platform.claude.com/docs/en/managed-agents/files)
- Build persistent memory (nav section header, no direct link captured)

**Advanced orchestration**
- [Multiagent sessions](https://platform.claude.com/docs/en/managed-agents/multi-agent) (this page)
...
- MCP tunnels (nav section header, no direct link captured)
...
This page documents Anthropic's server-hosted "Managed Agents" multiagent orchestration model (a coordinator agent delegating to a roster of sub-agents, each with isolated context/threads, shared sandbox and vault credentials). This is a cloud-hosted service pattern, not something that runs on-device, but the underlying *pattern* — a coordinator process that fans out to specialized sub-agents/threads, aggregates status, and exposes a single primary event stream — is directly relevant to Omni Claw's dual-agent memory/orchestration architecture. In particular the thread model (isolated context per delegated agent, condensed status on a primary stream, tool-permission events cross-posted to the primary thread) is a useful reference architecture if Omni Claw ever needs to coordinate multiple on-device or hybrid (on-device + cloud) agents rather than a single agent loop.
