...
ARCHITECTURE.md, RESUME.md, profiles/nav.yaml first),
...
\- In Termux: \`tailscale status\` isn't available (app owns it) —
verify

instead with \`curl http://\<any-tailnet-ip\>\` once a second node
exists.
...
JetPack 6.x: if the board is new/older firmware, first boot JetPack
5.1.3
...
\`curl -fsSL https://tailscale.com/install.sh \| sh && sudo tailscale
up\`
...
\- Success criteria: phone can \`git push\` vault to Jetson over
tailnet;
...
\- Do not push to any Novus-Agenti branch. aesop repo: main only, docs
only.

\- No tokens/secrets in any commit. HF auth comes from environment/HF
CLI login.

\- If a step's docs conflict with reality (firmware, image names),
verify

against the official source before flashing — report, don't improvise.

\- Log outcomes back into aesop/RESUME.md (append a dated section) so
the
